...
jQuery(document).ready(function($) {
	...
	$('.registerbox form').submit(function(e) {
		...
		if($(this).find('input[name=name]').attr('value') == '') {
			alert('Please enter your name.');
			return false;
		}
		
		if($(this).find('input[name=email]').attr('value') == '') {
			alert('Please enter your email address.');
			return false;
		}
		
		var emailRegex = /^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i;
		if($(this).find('input[name=email]').attr('value').search(emailRegex) == -1) {
		...
		$.post(
			...
			{
				listname: 'highlighter',
				name: $(this).find('input[name=name]').attr('value'),
				email: $(this).find('input[name=email]').attr('value')
			}
		);
		...
		$.post(
			ajaxurl,
			{
				action: 'ANNOtype_register',
			},
			function(data) {
				window.location = data + '?page=Highlighter';
			}
		);
	});
